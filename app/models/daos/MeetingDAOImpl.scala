...
class EventDAOImpl @Inject()(protected val dbConfigProvider: DatabaseConfigProvider) extends EventDAO {
  ...
  override def myEvents(user: User, start: LocalDateTime, end: LocalDateTime) = {
    ...
    val query = eventTable
      .filter(
        m =>
          (if (user.userType == UserType.Tutee)
             m.tuteeID.nonEmpty && m.tuteeID.getOrElse(UUID.randomUUID()) === user.userID
           else m.tutorID === user.userID)
  ...
  override def tutorEvents(tuteeID: UUID, tutorID: UUID, start: LocalDateTime, end: LocalDateTime) = {
    val query = eventTable
      .filter(
        m =>
          (m.tutorID === tutorID || m.tuteeID.getOrElse(UUID.randomUUID()) === tuteeID)
            && m.eventType =!= EventType.Canceled
            && m.eventType =!= EventType.Declined
            && m.start <= end && m.end >= start)
    db.run(query.result)
  ...
  override def getTuteeEventDetails(eventID: Long) = {
    implicit val getTuteeEventDetailsResult = GetResult(
      r => TuteeEventDetails(r.<<, r.<<, r.<<, r.<<, r.<<, r.<<, r.<<, r.<<, r.<<))
    val query = sql"""
                  select u.first_name, u.last_name, p.learning_language, p.subject_improve_1, p.score_subject_improve_1, p.subject_improve_2, p.score_subject_improve_2, e.description, e.event_type
                  from users u
  ...
  override def getTutorEventDetails(eventID: Long) = {
    implicit val getTutorEventDetailsResult = GetResult(
      r => TutorEventDetails(r.<<, r.<<, r.<<, r.<<, r.<<, r.<<, r.<<, r.<<, r.<<))
    val query = sql"""
                  select u.first_name, u.last_name, p.teaching_language, p.subject_1, p.subject_2, p.subject_3, p.subject_4, e.description, e.event_type
                  from users u
